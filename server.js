  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    h1 {
      font-size: 48px;
      margin-bottom: 30px;
      background: linear-gradient(90deg, #fff 0%, #ff0066 25%, #00ff99 50%, #3399ff 75%, #fff 100%);
    ...
    body.light-mode h1 {
      background: linear-gradient(90deg, #000 0%, #ff0066 25%, #00cc88 50%, #3366ff 75%, #000 100%);
      background-size: 200% auto;
      -webkit-background-clip: text;
      -webkit-text-fill-color: transparent;
      animation: flow 4s linear infinite;
...
<body>
  ...
  <div class="mode-toggle" id="modeToggle">Dark / Light</div>
  <div class="container">
    <h1>RAINBOW PROXY</h1>
    <div class="input-wrapper">
      <input id="urlInput" placeholder="enter url (e.g. example.com)" autocomplete="off">
    </div>
    <br>
  ...
  <script>
    ...
    modeToggle.addEventListener('click', () => {
      ...
      modeToggle.textContent = lightMode ? 'Light / Dark' : 'Dark / Light';
    });
    ...
    const frameContainer = document.getElementById('proxyFrameContainer');
    ...
    function go() {
      ...
      frame.src = proxyUrl;
      setTimeout(() => frameContainer.scrollIntoView({ behavior: 'smooth', block: 'center' }), 150);
    }
    ...
    // Optional: Rewrite links in iframe
...
app.use('/proxy', (req, res, next) => {
  ...
  createProxyMiddleware({
    ...
    onProxyRes: (proxyRes, req, res) => {
      ...
      ['x-frame-options', 'content-security-policy', 'x-content-type-options', 'strict-transport-security', 'frame-options'].forEach(h => {
